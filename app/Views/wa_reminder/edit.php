...
<div class="row">
  <div class="col-12 col-lg-7">
    <div class="card">
      <div class="card-body">
        <h5 class="card-title fw-semibold mb-4">Edit Template WA</h5>

        <form action="<?= base_url("admin/wa-reminder/{$template['id']}") ?>" method="post">
          <?= csrf_field() ?>
          ...
          <div class="mb-3">
            ...
            <input type="text" name="template_name"
              ...
              value="<?= esc(old('template_name', $template['template_name'])) ?>">
          ...
          <div class="mb-3">
            ...
            <select name="type" class="form-select <?= $validation->hasError('type') ? 'is-invalid' : '' ?>">
              <option value="before_due" <?= old('type', $template['type']) === 'before_due' ? 'selected' : '' ?>>
                H-1 (Sehari sebelum jatuh tempo)
              </option>
              <option value="overdue" <?= old('type', $template['type']) === 'overdue' ? 'selected' : '' ?>>
          ...
          <div class="mb-3">
            ...
            <textarea name="message_template" id="messageTemplate" rows="9"
              class="form-control <?= $validation->hasError('message_template') ? 'is-invalid' : '' ?>"><?= esc(old('message_template', $template['message_template'])) ?></textarea>
